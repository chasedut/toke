...
func TestGrepWithIgnoreFiles(t *testing.T) {
	...
	// Create .tokeignore file
	tokeignoreContent := "node_modules/\n"
	require.NoError(t, os.WriteFile(filepath.Join(tempDir, ".tokeignore"), []byte(tokeignoreContent), 0o644))
	...
	// node_modules/lib.js should be ignored by .tokeignore
...
func TestSearchImplementations(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(filepath.Join(tempDir, ".tokeignore"), []byte("file5.txt\n"), 0o644))
