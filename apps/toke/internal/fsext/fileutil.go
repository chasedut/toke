...
func NewFastGlobWalker(searchPath string) *FastGlobWalker {
	...
	// Load tokeignore if it exists
	tokeignorePath := filepath.Join(searchPath, ".tokeignore")
	if _, err := os.Stat(tokeignorePath); err == nil {
		if ci, err := ignore.CompileIgnoreFile(tokeignorePath); err == nil {
			walker.tokeignore = ci
		}
	}
...
// ShouldSkip checks if a path should be skipped based on gitignore, tokeignore, and hidden file rules
func (w *FastGlobWalker) ShouldSkip(path string) bool {
	...
	if w.tokeignore != nil {
		if w.tokeignore.MatchesPath(relPath) {
			return true
		}
	}
