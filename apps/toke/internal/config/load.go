...
func Load(workingDir string, debug bool) (*Config, error) {
	...
	// Fix local provider port for MLX models
	if err := cfg.fixLocalProviderPort(); err != nil {
		slog.Warn("Failed to fix local provider port", "error", err)
	}
...
func PushPopTokeEnv() func() {
	found := []string{}
	for _, ev := range os.Environ() {
		if strings.HasPrefix(ev, "TOKE_") {
			pair := strings.SplitN(ev, "=", 2)
			if len(pair) != 2 {
				continue
			}
			found = append(found, strings.TrimPrefix(pair[0], "TOKE_"))
		}
	}
	...
	for _, ev := range found {
		os.Setenv(ev, os.Getenv("TOKE_"+ev))
	}
...
func globalConfig() string {
	...
	// for windows, it should be in `%LOCALAPPDATA%/toke/`
	// for linux and macOS, it should be in `$HOME/.config/toke/`
...
func GlobalConfigData() string {
	...
	// for windows, it should be in `%LOCALAPPDATA%/toke/`
	// for linux and macOS, it should be in `$HOME/.local/share/toke/`
	if runtime.GOOS == "windows" {
		localAppData := os.Getenv("LOCALAPPDATA")
		if localAppData == "" {
			localAppData = filepath.Join(os.Getenv("USERPROFILE"), "AppData", "Local")
