...
// Config holds the configuration for toke.
type Config struct {
...
// IsConfigured return true if at least one provider is configured with an API key or is local
func (c *Config) IsConfigured() bool {
	// Check if any provider has an actual API key configured (not just loaded from provider list)
	for _, provider := range c.EnabledProviders() {
		if provider.APIKey != "" && !strings.HasPrefix(provider.APIKey, "$") {
			// Provider has a real API key (not just an env var placeholder)
			return true
		}
