...
func (c *Config) fixLocalProviderPort() error {
	...
	// Determine backend type more robustly than just looking for "mlx" in ID
	modelID := localConfig.ModelID

	// Heuristic 1: Endpoint already encodes the correct port
	isMLX := strings.Contains(localConfig.Endpoint, ":11435")
	// Heuristic 2: Check on-disk model layout: ~/.toke/models/mlx/<modelID>
	if !isMLX {
		dataRoot := filepath.Dir(c.dataConfigDir)
		mlxDir := filepath.Join(dataRoot, "models", "mlx", modelID)
		if fi, err := os.Stat(mlxDir); err == nil && fi.IsDir() {
			isMLX = true
		}
	}
	// Heuristic 3: Fallback to model ID substring
	if !isMLX {
