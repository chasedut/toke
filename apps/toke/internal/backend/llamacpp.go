...
func (b *LlamaCppBackend) DownloadServer(ctx context.Context, progressFn func(downloaded, total int64)) error {
	...
	if HasEmbeddedBackends() {
		if err := ExtractEmbeddedBackends(b.dataDir); err != nil {
			...
			// Check for extracted llama-server
			embeddedPath := GetEmbeddedBackendPath(b.dataDir, "llama-server")
			if _, err := os.Stat(embeddedPath); err == nil {
				b.binaryPath = embeddedPath
				slog.Info("Using embedded llama-server", "path", embeddedPath)
				return nil
			}
		}
	}
	
	// Check for bundled llama-server in executable directory
	if execPath, err := os.Executable(); err == nil {
		execDir := filepath.Dir(execPath)
		bundledPath := filepath.Join(execDir, "backends", "llama-server")
		if _, err := os.Stat(bundledPath); err == nil {
			b.binaryPath = bundledPath
			slog.Info("Using bundled llama-server", "path", bundledPath)
			return nil
		}
	}
