...
// LaunchLocalSetupAutoMsg is sent when user wants to set up local models with auto-selection
type LaunchLocalSetupAutoMsg struct{}
...
func (m *modelDialogCmp) listHeight() int {
	if m.wHeight == 0 {
		// Conservative default to prevent overflow on initial render
		return 5
	}
	// Reserve space for dialog chrome (title, borders, help, padding)
	availableHeight := m.wHeight - 10
	// Use at most 2/3 of available height, cap at 20 lines
