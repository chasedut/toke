...
// Additional message types not in the main list above
type (
...
func (c *commandDialogCmp) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.WindowSizeMsg:
		...
		// Only resize the list, don't reload commands
...
func (c *commandDialogCmp) Position() (int, int) {
	...
	// Estimate height based on typical command list size
	dialogHeight := min(20, c.wHeight/2) + 6 // list height + borders/padding
	row := (c.wHeight - dialogHeight) / 2
	
	// Ensure dialog doesn't go off bottom
	maxRow := c.wHeight - dialogHeight - 3
	if row > maxRow {
		row = maxRow
	}
	if row < 2 {
...
func (c *commandDialogCmp) defaultCommands() []Command {
	...
	return append(commands, []Command{
		...
		},
		{
			...
			Description: "Create/Update the TOKE.md memory file",
			Handler: func(cmd Command) tea.Cmd {
		...
		},
		{
			...
			Description: "Clear all saved configuration and start fresh",
			Handler: func(cmd Command) tea.Cmd {
				return util.CmdHandler(ResetConfigurationMsg{})
			},
		},
		{
			ID:          "quit",
