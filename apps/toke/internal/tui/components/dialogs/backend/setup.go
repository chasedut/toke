...
// DEPRECATED - DO NOT USE - Use NewLocalModelDialogCmp instead
func NewDeprecated(orchestrator *backend.Orchestrator, onComplete func(*backend.ModelOption), onSkip func(), onAPIKey func()) Model {
...
func NewLocalOnlyDeprecated(orchestrator *backend.Orchestrator, onComplete func(*backend.ModelOption), onSkip func()) Model {
	searchInput := textinput.New()
	searchInput.Placeholder = "Search for models (e.g., 'llama', 'mistral', 'qwen')..."
	searchInput.CharLimit = 100
...
func (m Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.WindowSizeMsg:
		slog.Debug("Backend dialog: Received WindowSizeMsg", "width", msg.Width, "height", msg.Height, "prev_width", m.width, "prev_height", m.height)
		m.width = msg.Width
		m.height = msg.Height
		// Set progress bar width to be responsive
		progressWidth := min(52, msg.Width - 28)  // Responsive width with padding
		if progressWidth > 0 {
		...
		default:
			...
			if m.selectedOption >= 3 {
				...
				if advancedIdx >= 3 && advancedIdx <= 6 {
					...
					m.selectedModel = backend.GetRecommendedByTier(backend.TierMidShelf)
					m.state = StateDownloading
					return m, m.startDownload()
...
func (m Model) View() string {
	...
	case StateHFBrowse:
		content = m.renderHFBrowse()
	case StateHFSearch:
		content = m.renderHFSearch()
	case StateHFFileSelect:
		content = m.renderHFFileSelect()
	case StateDownloading:
		content = m.renderDownloadingAnimated()
	case StateComplete:
	...
	// Apply dialog styling only for the download/complete states
	if m.state == StateDownloading || m.state == StateComplete {
		return m.dialogStyle().Render(content)
	}
	return content
}

// dialogStyle returns the style for the dialog border
func (m Model) dialogStyle() lipgloss.Style {
	...
	// Max width of 70, but responsive to smaller terminals
	dialogWidth := min(70, width - 10)  // Leave 10 chars margin
...
func (m Model) Position() (int, int) {
	// Use reasonable defaults if dimensions not set yet
	width := m.width
	height := m.height
	if width == 0 {
		slog.Debug("Backend dialog Position: width is 0, using default 80")
		width = 80  // More conservative default
	}
	if height == 0 {
		slog.Debug("Backend dialog Position: height is 0, using default 24")
		height = 24  // More conservative default
	}
	
	// Calculate actual dialog width (responsive)
	dialogWidth := min(70, width - 10) + 10  // content + padding + border
	
	// Estimate dialog height based on content
	// The download dialog is roughly 20-25 lines tall
	dialogHeight := 25
	
	// Center vertically, but ensure it fits within the window
	row := (height - dialogHeight) / 2
	
	// Make sure dialog doesn't go off screen
	maxRow := height - dialogHeight - 2  // Leave 2 lines at bottom for status
	if row > maxRow {
		row = maxRow
	}
	if row < 1 {
	...
	// Center horizontally
	col := (width - dialogWidth) / 2
	if col < 2 {
		col = 2
	}
	
	slog.Debug("Backend dialog Position", "terminal_width", width, "terminal_height", height, 
		"dialog_width", dialogWidth, "dialog_height", dialogHeight, "row", row, "col", col)
