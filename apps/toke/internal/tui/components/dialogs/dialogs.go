...
func (d dialogCmp) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.WindowSizeMsg:
		var cmds []tea.Cmd
		slog.Debug("DialogCmp: Received WindowSizeMsg", "width", msg.Width, "height", msg.Height, "prev_width", d.width, "prev_height", d.height)
		d.width = msg.Width
		d.height = msg.Height
		for i := range d.dialogs {
			slog.Debug("DialogCmp: Forwarding WindowSizeMsg to dialog", "dialog_id", d.dialogs[i].ID())
	...
	case OpenDialogMsg:
		slog.Debug("DialogCmp: Opening dialog", "dialog_id", msg.Model.ID(), "width", d.width, "height", d.height)
...
func (d dialogCmp) handleOpen(msg OpenDialogMsg) (tea.Model, tea.Cmd) {
	...
	// Only send WindowSizeMsg if we have valid dimensions
	// Otherwise use reasonable defaults for initial render
	width := d.width
	height := d.height
	if width == 0 || height == 0 {
		slog.Debug("DialogCmp: No dimensions yet, using defaults and requesting window size")
		// Use terminal defaults if we haven't received WindowSizeMsg yet
		width = 80
		height = 24
		// Also queue a command to get the actual window size
		cmds = append(cmds, func() tea.Msg { return tea.RequestWindowSize() })
	}
	
	slog.Debug("DialogCmp: Sending initial WindowSizeMsg to dialog", "dialog_id", msg.Model.ID(), "width", width, "height", height)
	updatedModel, cmd := msg.Model.Update(tea.WindowSizeMsg{
		Width:  width,
		Height: height,
	})
	// IMPORTANT: Update the dialog in our list with the updated model that has the new width/height
	d.dialogs[len(d.dialogs)-1] = updatedModel.(DialogModel)
	cmds = append(cmds, cmd)
	return d, tea.Batch(cmds...)
}
