...
type splashCmp struct {
	...
	showModelChoice  bool   // Show local vs cloud choice
	selectedChoice   int    // 0 = local, 1 = cloud, 2 = use existing key
	detectedAPIKeys  map[string]string // Provider -> key prefix
}
...
func (s *splashCmp) SetOnboarding(onboarding bool) {
	...
	if onboarding {
		s.showModelChoice = true // Start with local vs cloud choice
		s.detectAPIKeys()
		// If API keys are detected, default to using them, otherwise local
		if len(s.detectedAPIKeys) > 0 {
			s.selectedChoice = 0 // Use existing key
		} else {
			s.selectedChoice = 0 // Local
		}
	}
}

func (s *splashCmp) detectAPIKeys() {
	s.detectedAPIKeys = make(map[string]string)
	...
	for envVar, provider := range checks {
		if key := os.Getenv(envVar); key != "" {
			// Store first 8 chars of the key for display
			prefix := key
			if len(key) > 8 {
				prefix = key[:8] + "..."
			}
			s.detectedAPIKeys[provider] = prefix
		}
	}
}
...
func (s *splashCmp) SetSize(width int, height int) tea.Cmd {
    // Respect the actual available size; do not upsize beyond container
    // Dimensions may be small initially before the first resize
	
	wasSmallScreen := s.isSmallScreen()
	rerenderLogo := width != s.width || s.logoRendered == ""
	s.height = height
	s.width = width
	...
	computedListHeight := s.height - lipgloss.Height(s.logoRendered) - (SplashScreenPaddingY * 2) - s.logoGap() - 2
	if computedListHeight < 0 {
		computedListHeight = 0
	}
	s.listHeight = computedListHeight
...
func (s *splashCmp) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyPressMsg:
		...
		case key.Matches(msg, s.keyMap.Select):
			// Handle model choice screen
			if s.showModelChoice {
				s.showModelChoice = false
				if len(s.detectedAPIKeys) > 0 {
					// With API keys detected
					switch s.selectedChoice {
					case 0:
						// Use existing API key - show cloud models for that provider
						// Continue with regular onboarding flow
						return s, nil
					case 1:
						// Local model selected - launch backend setup with auto-selection
						s.isOnboarding = false
						return s, tea.Batch(
							util.CmdHandler(OnboardingCompleteMsg{}),
							util.CmdHandler(models.LaunchLocalSetupAutoMsg{}),
						)
					case 2:
						// Different cloud provider - show all cloud models
						// Continue with regular onboarding flow
						return s, nil
					}
				} else {
					// No API keys detected
					if s.selectedChoice == 0 {
						// Local model selected - launch backend setup with auto-selection
						s.isOnboarding = false
						return s, tea.Batch(
							util.CmdHandler(OnboardingCompleteMsg{}),
						...
						// Cloud model selected - show cloud model list
						// Continue with regular onboarding flow
						return s, nil
					}
				}
			...
			if s.isOnboarding && !s.needsAPIKey && !s.showModelChoice {
		...
		case key.Matches(msg, s.keyMap.Tab, s.keyMap.LeftRight):
			if s.showModelChoice {
				...
				if len(s.detectedAPIKeys) > 0 {
					// 3 choices: use key, local, cloud
					s.selectedChoice = (s.selectedChoice + 1) % 3
				} else {
					// 2 choices: local, cloud
					s.selectedChoice = 1 - s.selectedChoice
				}
				return s, nil
			}
			if s.needsAPIKey {
		...
		case key.Matches(msg, s.keyMap.Previous, s.keyMap.Next):
			if s.showModelChoice {
				// Cycle through choices
				if len(s.detectedAPIKeys) > 0 {
					// 3 choices: use key, local, cloud
					if key.Matches(msg, s.keyMap.Previous) {
						s.selectedChoice = (s.selectedChoice + 2) % 3 // Go backward
					} else {
						s.selectedChoice = (s.selectedChoice + 1) % 3 // Go forward
					}
				} else {
					// 2 choices: local, cloud
					s.selectedChoice = 1 - s.selectedChoice
			...
			// Fall through to regular handling for other states
		case key.Matches(msg, s.keyMap.Yes):
			...
			if s.isOnboarding && !s.showModelChoice {
		...
		default:
			if s.needsAPIKey {
				...
				s.apiKeyInput = u.(*models.APIKeyInput)
				return s, cmd
			} else if s.isOnboarding && !s.showModelChoice {
				u, cmd := s.modelList.Update(msg)
				s.modelList = u
				return s, cmd
			}
...
func (s *splashCmp) View() string {
	...
	if s.needsAPIKey {
		...
		if s.showModelChoice {
			// Show local vs cloud choice
			titleStyle := t.S().Base.Foreground(t.Primary).Bold(true)
			selectedStyle := t.S().Base.Foreground(t.Success).Background(t.BgSubtle).PaddingLeft(1).PaddingRight(1)
			unselectedStyle := t.S().Base.Foreground(t.FgMuted).PaddingLeft(1).PaddingRight(1)
			
			localStyle := unselectedStyle
			cloudStyle := unselectedStyle
			if s.selectedChoice == 0 {
				localStyle = selectedStyle
			} else {
				cloudStyle = selectedStyle
			}
			
			var choiceView string
			if len(s.detectedAPIKeys) > 0 {
				// Show detected API keys
				var keyInfo string
				for provider, keyPrefix := range s.detectedAPIKeys {
					if keyInfo != "" {
						keyInfo += ", "
					}
					keyInfo += fmt.Sprintf("%s (%s)", provider, keyPrefix)
				}
				
				useKeyStyle := unselectedStyle
				localStyle := unselectedStyle
				cloudStyle := unselectedStyle
				
				switch s.selectedChoice {
				case 0:
					useKeyStyle = selectedStyle
				case 1:
					localStyle = selectedStyle
				case 2:
					cloudStyle = selectedStyle
				...
				choiceContent := lipgloss.JoinVertical(
					lipgloss.Left,
					titleStyle.Render("How would you like to run your AI?"),
					"",
					t.S().Base.Foreground(t.Success).Render("🔑 Detected API Keys: " + keyInfo),
					"",
					useKeyStyle.Render("→ Use Existing API Key") + t.S().Base.Foreground(t.FgMuted).Render(" - Use your detected API key for cloud models"),
					"",
					localStyle.Render("  Setup Local Model") + t.S().Base.Foreground(t.FgMuted).Render(" - Download and run AI models on your machine (free, private)"),
					"",
					cloudStyle.Render("  Setup Different Cloud Provider") + t.S().Base.Foreground(t.FgMuted).Render(" - Configure a different API provider"),
					"",
					"",
					t.S().Base.Foreground(t.FgSubtle).Render("Use ↑/↓ or Tab to switch • Enter to select"),
				)
				
				// Add border like command palette
				choiceView = t.S().Base.
					Border(lipgloss.RoundedBorder()).
					BorderForeground(t.BorderFocus).
					Padding(2).
					Render(choiceContent)
			} else {
				// No API keys detected - original view
				choiceContent := lipgloss.JoinVertical(
					lipgloss.Left,
					titleStyle.Render("How would you like to run your AI?"),
					"",
					"",
					localStyle.Render("→ Local Model") + t.S().Base.Foreground(t.FgMuted).Render(" - Free, private, no data leaves your computer"),
					"",
					cloudStyle.Render("  Cloud Model") + t.S().Base.Foreground(t.FgMuted).Render(" - API key required, faster responses"),
					"",
					"",
					t.S().Base.Foreground(t.FgSubtle).Render("Use ↑/↓ or Tab to switch • Enter to select"),
				)
			...
			// Center the choice view both vertically and horizontally
			choiceSelector := t.S().Base.
			...
			modelSelector := t.S().Base.AlignVertical(lipgloss.Bottom).Height(remainingHeight).Render(
				lipgloss.JoinVertical(
					lipgloss.Left,
					t.S().Base.PaddingLeft(1).Foreground(t.Primary).Render("Choose a Cloud Model"),
					"",
					modelListView,
				),
			)
		...
		initText := lipgloss.JoinVertical(
			...
			bodyStyle.Render("result into a TOKE.md file which serves as general context."),
			"",
	...
	// Height() applies to the content box; since we also add top/bottom padding,
	// reduce the target height so the total rendered block stays within s.height
	contentHeight := s.height - (SplashScreenPaddingY * 2)
	if contentHeight < 0 {
		contentHeight = 0
	}
	return t.S().Base.
