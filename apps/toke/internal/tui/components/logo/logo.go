...
func Render(version string, compact bool, o Opts) string {
	...
	// Narrow version.
	if compact {
		// Each emoji takes 2 cells in terminal, so divide by 4 instead of 2
		fieldCount := max(1, tokeWidth/4)
		field := fg(o.FieldColor, strings.Repeat(diag, fieldCount))
	...
	// Each emoji takes 2 cells, so adjust calculations
	rightWidth := max(3, (o.Width-tokeWidth-leftWidth*4-2)/4)
	const stepDownAt = 0
...
func SmallRender(width int) string {
	...
	if remainingWidth > 0 {
		// Each emoji takes 2 cells, so divide by 2 for the number of emojis
		emojiCount := max(1, remainingWidth/4)
		lines := strings.Repeat("🍃", emojiCount)
