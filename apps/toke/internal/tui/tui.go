...
// DelayedLaunchLocalSetupMsg is sent after a delay to open the local model setup dialog
type DelayedLaunchLocalSetupMsg struct{}

// DelayedLaunchLocalSetupAutoMsg is sent after a delay to open the local model setup with auto-selection
type DelayedLaunchLocalSetupAutoMsg struct{}
...
func (a *appModel) Init() tea.Cmd {
	...
	// This handles cases where the terminal isn't fully initialized yet
...
func (a *appModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	if a.isLoading {
		...
		case InitializationCompleteMsg:
			...
			slog.Info("InitializationCompleteMsg - transitioning from loading",
				"current_wWidth", a.wWidth, "current_wHeight", a.wHeight,
				"current_width", a.width, "current_height", a.height)
			a.isLoading = false
	...
	case tea.KeyboardEnhancementsMsg:
		...
		return a, tea.Batch(cmds...)
	case tea.WindowSizeMsg:
		slog.Info("WindowSizeMsg in main update",
			"raw_width", msg.Width, "raw_height", msg.Height,
	...
	case commands.AddNewModelsMsg:
		// Launch the new local models dialog
	...
	case commands.ResetConfigurationMsg:
		// Show confirmation dialog for reset
		return a, util.CmdHandler(dialogs.OpenDialogMsg{
			Model: NewResetConfigDialog(),
		})
	...
	case models.LaunchLocalSetupMsg:
		...
		// This gives the TUI time to establish proper dimensions
	...
	case DelayedLaunchLocalSetupMsg:
		...
		// Debug: Log that we're opening the new dialog
		dialog := models.NewLocalModelDialogCmp()
		if dialog == nil {
			return a, util.ReportError(fmt.Errorf("Failed to create local models dialog"))
		}
		slog.Info("Opening local model dialog after delay",
	...
	case DelayedLaunchLocalSetupAutoMsg:
		...
		dialog := models.NewLocalModelDialogAutoDownload()
		if dialog == nil {
			return a, util.ReportError(fmt.Errorf("Failed to create auto-download dialog"))
		}
		slog.Info("Opening auto-download dialog",
			"width", a.width, "height", a.height)
		return a, util.CmdHandler(dialogs.OpenDialogMsg{
			Model: dialog,
		})
...
func (a *appModel) handleWindowResize(width, height int) tea.Cmd {
	...
	slog.Info("handleWindowResize",
		"input_width", width, "input_height", height+statusHeight,
		"statusHeight", statusHeight,
		"new_width", a.width, "new_height", a.height,
